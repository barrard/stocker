...
function get_minutely(symbol, count, time_frame, cb) {
...
function get_historical(symbol, count, cb) {
  console.log(`get ${count} bars of ${symbol}`)
  if (minute_data[symbol]) {
    // if (typeof minute_data !== 'undefined' && minute_data[symbol] != 'undefined') {
    console.log(minute_data[symbol])
    cb(minute_data[symbol].slice(count * -1))
  } else {
    var url = `http://localhost:55555/stock/${symbol}/historical`

    $.get(url, (resp) => {
      console.log(resp)
      var data = resp.resp[0].minutely_data
      minute_data[symbol] = data
      cb((data.slice(count * -1)))
